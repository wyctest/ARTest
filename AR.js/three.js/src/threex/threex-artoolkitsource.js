	...
	navigator.mediaDevices.enumerateDevices().then(function(devices) {
		...
		for (var i = 0; i != devices.length; ++i) { 
			 ...
			 exArray.push(sourceInfo.deviceId); 
				...
				deviceId: exArray[1], //0为默认摄像头，1为后置 
		...
		navigator.mediaDevices.getUserMedia(userMediaConstraints).then(function success(stream) {
			...
			// to start the video, when it is possible to start it only on
			// userevent. like in android
...
ARjs.Source.prototype.onResizeElement = function(mirrorDomElements){
	...
	// if screenAspect < sourceAspect, then change the width, else change the
	// height
	...
	// if( mirrorDomElements !== undefined ) console.warn('still use the old
	// resize. fix it')
